...
def ortho_basis_from_normal(n: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
    n = normalize(n)
    up = np.array([0.0,0.0,1.0])
    if abs(float(np.dot(up, n))) > 0.9:
        up = np.array([1.0,0.0,0.0])
    e1 = normalize(np.cross(n, up)); e2 = np.cross(n, e1)
    return e1, e2

def rot_axis_angle(u: np.ndarray, axis: np.ndarray, angle: float) -> np.ndarray:
    axis = normalize(axis); c, s = math.cos(angle), math.sin(angle)
    return u*c + np.cross(axis, u)*s + axis*np.dot(axis, u)*(1-c)
...
def log_map(c: np.ndarray, p: np.ndarray, R: float) -> Tuple[float, np.ndarray]:
    ...
    e1, e2 = ortho_basis_from_normal(c)
...
def polygons_overlap_SAT(offset: np.ndarray, V1: np.ndarray, V2: np.ndarray) -> bool:
    V2s = V2 + offset
    axes = np.vstack([polygon_axes_from_edges(polygon_edges(V1)),
...
def regular_polygon_vertices(n: int, side_nm: float) -> np.ndarray:
    ...
    ang0 = math.pi/n
    return np.array([[R*math.cos(ang0+2*k*math.pi/n),
                      R*math.sin(ang0+2*k*math.pi/n)] for k in range(n)], dtype=float)
...
# ========= 单向吸附 MC 主过程（任意凸多边形脚印，支持历史返回）=========
def adsorption_polygons_on_sphere(R_nm: float, poly_base: np.ndarray, seed: int = 0,
                           max_attempts: int = 200_000, stagnation_limit: int = 20_000,
                           progress: bool = True, progress_every: int = 5_000,
    ...
    def bin_index(dir_vec: np.ndarray):
        theta = math.acos(np.clip(dir_vec[2], -1.0, 1.0))
        i = int(theta / dtheta);  i = min(J-1, max(0, i))
        phi = math.atan2(dir_vec[1], dir_vec[0]);  phi = phi if phi>=0 else (phi+2*math.pi)
        k = int(phi / dphi);      k = min(K-1, max(0, k))
        return i, k, theta

    def neighbor_bins_metric(i: int, k: int, theta: float):
    ...
    def neighbor_bins_by_angle(i: int, k: int, theta: float, angle: float):
        sin_th = math.sin(theta)
    ...
    def reorthonormalize_tangent(c: np.ndarray, e1: np.ndarray, e2: np.ndarray) -> Tuple[np.ndarray,np.ndarray]:
        ...
        if n1 < 1e-20:
            # 退化时重建
            e1p, e2p = ortho_basis_from_normal(c)
            return e1p, e2p
        e1p = e1p / n1
        e2p = np.cross(c, e1p)
    ...
    def overlap_between_configs(cA: np.ndarray, e1A: np.ndarray, e2A: np.ndarray, phiA: float,
                                 cB: np.ndarray, e1B: np.ndarray, e2B: np.ndarray, phiB: float) -> bool:
        ...
        B2 = np.array([[float(np.dot(u2, e1A)), float(np.dot(v2, e1A))],
                       [float(np.dot(u2, e2A)), float(np.dot(v2, e2A))]], dtype=float)
        V2 = (poly_base @ B2)
    ...
    def attempt_local_relax(cC: np.ndarray, e1C: np.ndarray, e2C: np.ndarray, phiC: float,
                            iC: int, kC: int, thetaC: float, rng: np.random.Generator) -> bool:
        # 参数换算
        ...
        for it in range(int(relax_max_iter)):
            ...
            for qid in O:
                ...
                for act, val in actions:
                    ...
                    if act == "move":
                        # 回退预算
                        alpha_mv = math.acos(float(np.clip(np.dot(c_old, c_old), -1.0, 1.0)))  # 0.0
                        # 无需调整预算（我们只在接受时累计）
                        pass

                # end for actions
                # 若该 q 无法接受任何动作，继续尝试其它 q
                _ = accepted_this_q
    ...
    while attempts < max_attempts and streak < stagnation_limit:
        ...
        for idx in cand_ids:
            ...
            B2 = np.array([[float(np.dot(u2, e1)), float(np.dot(v2, e1))],
                           [float(np.dot(u2, e2)), float(np.dot(v2, e2))]], dtype=float)
